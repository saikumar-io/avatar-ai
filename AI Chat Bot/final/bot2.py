...
async def handle_response(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current_question_index + 1 < len(question_list):
        ...
        # Translate and speak "Thanks!" message
        ...
        if user_language != "en-IN":
            try:
                ...
                # Convert thanks message to speech
                thanks_audio_base64 = text_to_speech(localized_thanks, user_language)
                
                # Save and send audio
                if thanks_audio_base64:
                    thanks_audio_path = await save_audio(thanks_audio_base64, f"{user_id}_thanks.wav")
                    if thanks_audio_path:
                        with open(thanks_audio_path, 'rb') as audio_file:
                            await update.message.reply_voice(audio_file)
